...
namespace u255 {
    ...
    func add_u512_and_u256{range_check_ptr}(a: u512, b: Uint256) -> u512 {
        alloc_locals;
    ...
    func mul{range_check_ptr}(a: Uint256, b: Uint256) -> u512 {
        alloc_locals;
        ...
        assert carry = 0;
        let res = u512(res0, res2, res4, res6);
        return res;
    }
    func square{range_check_ptr}(a: Uint256) -> u512 {
        alloc_locals;
        let (a0, a1) = split_64(a.low);
        let (a2, a3) = split_64(a.high);
        ...
        let (res4, carry) = split_128((a3 * a12) * 2 + a2 * a2 + carry);

        let res = u512(res0, res2, res4, a3 * a3 + carry);
        return res;
    }

    func mul2ab{range_check_ptr}(a: Uint256, b: Uint256) -> u512 {
        alloc_locals;
        ...
        // let (a0a1, carry0) = split_128(a1 * 2 ** 65 + 2 * a0);
        // let (a2a3, carry1) = split_128(a3 * b0 + 2 * a2 + carry0);
        ...
        // let a2 = a2 * 2;  // + carry1;
        // let a3 = a3 * 2;
        // %{ print_felt_info(ids.a0,'ta0') %}
        // %{ print_felt_info(ids.a1,'ta1') %}
        // %{ print_felt_info(ids.a2,'ta2') %}
        // %{ print_felt_info(ids.a3,'ta3') %}

        local B0 = b0 * HALF_SHIFT;  // + 2 ** 65;
        local b12 = b1 + b2 * HALF_SHIFT;

        let (res0, carry) = split_128(a1 * B0 + a0 * b.low);  // OK
        let (res2, carry) = split_128(a3 * B0 + a2 * b.low + a1 * b12 + a0 * b.high + carry);
        let (res4, carry) = split_128(a3 * b12 + a2 * b.high + a1 * b3 + carry);
        let (res6, carry) = split_128(a3 * b3 + carry);
        assert carry = 0;
        // assert res6 = 0;
        // assert res4 = 0; fails at res4
        let res = u512(res0, res2, res4, res6);
        return res;
    }
    // Multiply by 2
    func double_u255{range_check_ptr}(a: Uint256) -> Uint256 {
        ...
        let (a2, a3) = split_64(a.high);
        local b0 = 2 ** 65;
        // %{ print_felt_info(ids.a0,'a0') %}
        // %{ print_felt_info(ids.a1,'a1') %}
        // %{ print_felt_info(ids.a2,'a2') %}
        // %{ print_felt_info(ids.a3,'a3') %}
        // local b12 = b1 + b2 * HALF_SHIFT;
        ...
        assert carry = 0;
        // let (a0, a1) = split_64(res0);
        // %{ print_felt_info(ids.a0,'na0') %}
        // %{ print_felt_info(ids.a1,'na1') %}
        // let (a2, a3) = split_64(res2);
        // %{ print_felt_info(ids.a2,'na2') %}
        // %{ print_felt_info(ids.a3,'na3') %}
        let res = Uint256(res0, res2);
        return res;
    }
    func double_u511{range_check_ptr}(a: u512) -> u512 {
        alloc_locals;
        ...
        assert carry = 0;
        let res = u512(d0=res0, d1=res2, d2=res4, d3=res6);
        return res;
    }
    func mul_u512_by_u256{range_check_ptr}(a: u512, b: Uint256) -> Uint768 {
        alloc_locals;
        let (a0, a1) = split_64(a.d0);
        let (a2, a3) = split_64(a.d1);
        let (a4, a5) = split_64(a.d2);
    ...
    func u768_modulo_p_25519{range_check_ptr}(a: Uint768) -> Uint256 {
        ...
        let res_mul: Uint768 = mul_u512_by_u256(
            u512(quotient.d0, quotient.d1, quotient.d2, quotient.d3), p
        );
    ...
    // replace unsigned div u256
    func a_modulo_p_255_19{range_check_ptr}(a: Uint256) -> Uint256 {
        alloc_locals;
        ...
        let res_mul: u512 = mul(quotient, div);
        assert res_mul.d2 = 0;
        assert res_mul.d3 = 0;
    ...
    func a_modulo_2_255_19{range_check_ptr}(a: Uint256) -> Uint256 {
        alloc_locals;
        // Guess the quotient and the remainder.
        local quotient: Uint256;
        local remainder: Uint256;
        // 2**255 + 19
        tempvar div = Uint256(19, 170141183460469231731687303715884105728);
        %{
            a = (ids.a.high << 128) + ids.a.low
            div = (ids.div.high << 128) + ids.div.low
            quotient, remainder = divmod(a, div)
    ...
    // func a_modulo_2_255{range_check_ptr}(a: Uint256) -> Uint256 {
    //     alloc_locals;
    //     // Guess the quotient and the remainder.
    //     local quotient: Uint256;
    //     local remainder: Uint256;
    //     // 2**255
    //     tempvar div = Uint256(0, 170141183460469231731687303715884105728);
    //     %{
    //         a = (ids.a.high << 128) + ids.a.low
    //         div = (ids.div.high << 128) + ids.div.low
    //         quotient, remainder = divmod(a, div)
    ...
    //     // uint256_check(quotient);
    //     // uint256_check(remainder);
    //     let (res_mul, carry) = mul(quotient, div);
    //     %{ print(f"carry={ids.carry.low}, {ids.carry.high}") %}
    //     assert carry = Uint256(1, 0);
    //     let res_mul = Uint256(res_mul.low, res_mul.high);

    // let check_val = add(res_mul, remainder);
    //     %{ print(f"check_val={ids.check_val.low}, {ids.check_val.high}") %}

    // assert check_val.low = a.low;
    //     assert check_val.high = a.high;
    //     // assert add_carry = 0;
