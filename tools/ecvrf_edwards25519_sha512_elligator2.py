...
def ecvrf_proof_to_hash(pi_string):
    ...
    beta_string = _hash(SUITE_STRING + three_string +
                        _encode_point(cofactor_gamma))
...
def ecvrf_verify(y, pi_string, alpha_string):
    ...
    if 'test_dict' in globals():
        _assert_and_sample(['h', 'u', 'v'], [
                           h, _encode_point(u), _encode_point(v)])
...
def _ecvrf_hash_points(p1, p2, p3, p4):
    ...
    string = string + \
        _encode_point(p1) + _encode_point(p2) + \
        _encode_point(p3) + _encode_point(p4)
